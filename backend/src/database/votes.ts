...
export async function getVoteById(vote_id: number): Promise<Vote> {
    const db = getDb();
    const result = await db.query(`SELECT * FROM votes WHERE id = $1`, [vote_id]);
    const row = result.rows[0];
    return rowToVote(row);
}

export async function getAllVotes(): Promise<FilledVote[]> {
    const db = getDb();
    const result = await db.query(`SELECT v.id, v.user_id, v.message_id, v.vote_type, v.datetime, u.name, m.message, m.response FROM votes v JOIN users u ON u.id = v.user_id JOIN messages m ON m.id = v.message_id`);
    return result.rows.map(rowToFilledVote);
}

export async function getFilledVote(user_id: number, message_id: number): Promise<FilledVote> {
    const db = getDb();
    const result = await db.query(
        `SELECT v.id, v.user_id, v.message_id, v.vote_type, v.datetime, u.name, m.message, m.response FROM votes v JOIN users u ON u.id = v.user_id JOIN messages m ON m.id = v.message_id WHERE v.user_id = $1 AND v.message_id = $2`,
        [user_id, message_id]
    )
    const row = result.rows[0];
...
export async function getFilledVotesByUser(user_id: number): Promise<FilledVote[]> {
    const db = getDb();
    const result = await db.query(
        `SELECT v.id, v.user_id, v.message_id, v.vote_type, v.datetime, u.name, m.message, m.response FROM votes v JOIN users u ON u.id = v.user_id JOIN messages m ON m.id = v.message_id WHERE v.user_id = $1`,
        [user_id]
    );
...
export async function voteExistsById(vote_id: number): Promise<boolean> {
    const db = getDb();
    const result = await db.query(`SELECT 1 FROM votes WHERE id = $1`, [vote_id]);
    return result.rows.length > 0;
}
...
export async function deleteVoteById(vote_id: number) {
    const db = getDb();
    const result = await db.query(`DELETE FROM votes WHERE id = $1`, [vote_id]);
    return result.rows.length > 0;
}
